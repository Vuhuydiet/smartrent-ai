...
async def get_listing_detail(
    ...
    try:
        data = await backend_client.get_listing(normalized_id)
        ...
        # Append the full listing object so the frontend gets all fields.
        ...
        if e.response.status_code == 404:
            return {
                "status": "error",
                "error": f"Listing {normalized_id} was not found.",
            }
        logger.error(
        ...
        logger.error(
            "get_listing_detail failed for %s: %s", normalized_id, e, exc_info=True
        )
